  ...
  fetch(`https://opensheet.elk.sh/${sheetID}/Sites`)
    ...
    .then((data) => {
      ...
      data.forEach((site, i) => {
        ...
        if (i === 0) {
          document
            .querySelector('.subnav[data-menu="sites"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<strong>${site.Status}</strong><a href="${site.URL}" target="_blank" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="characters"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<strong>${site.Status}</strong><a href="${prefix}/characters/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="threads"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<strong>${site.Status}</strong><a href="${prefix}/threads/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="stats"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<strong>${site.Status}</strong><a href="${prefix}/stats/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="writing"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<strong>${site.Status}</strong><a href="${prefix}/writing/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
          ...
          document
            ...
            .insertAdjacentHTML(
              "beforeend",
              `<a href="${site.URL}" target="_blank" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="characters"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<a href="${prefix}/characters/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="threads"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<a href="${prefix}/threads/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="stats"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<a href="${prefix}/stats/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
          document
            .querySelector('.subnav[data-menu="writing"] .subnav--inner')
            .insertAdjacentHTML(
              "beforeend",
              `<a href="${prefix}/writing/${site.ID}.html" class="${site.Status}">${site.Site}</a>`,
            );
        }
      });
    })
          ...
          fetch(`https://opensheet.elk.sh/${sheetID}/Writing`)
            ...
            .then((recordData) => {
              ...
              sites.forEach((site, i) => {
                let siteCharacters = storedCharacters.filter((item) =>
                  item.Sites.includes(site.Site),
                );
                let siteThreads = storedThreads.filter(
                  (item) => item.Site === site.Site,
                );
                let siteRecords = storedRecords.filter(
                  (item) => item.Site === site.Site,
                );
...
function initPartnerSelect(
  ...
  let site = el
    .closest("form")
    .querySelector(siteField)
    .options[
      el.closest("form").querySelector(siteField).selectedIndex
    ].innerText.trim()
    .toLowerCase();
  let partners = data.filter(
    (item) => item.Site === site && item.Status !== "inactive",
  );
  ...
  el.closest("form")
    .querySelectorAll("select#partner")
    .forEach((select) => {
      if (el.closest("form").dataset.form !== "edit-partner") {
        if (hasNPC) {
          select.addEventListener("change", (e) => {
            let target = e.currentTarget;
            let unselected = select
              .closest(".row")
              .querySelector(".ifUnselected");
      ...
      if (select.options.length < 2 || type === "refresh") {
        if (el.closest("form").dataset.form !== "edit-partner") {
          select.closest(".row").querySelector("#character").innerHTML =
            `<option value="">(select)</option>${hasNPC ? `<option value="npc">NPC</option>` : ``}`;
        }
...
function initShipSelect(e, siteField, data) {
  ...
  let site = e
    ...
    .querySelector(siteField)
    .options[
      e.closest("form").querySelector(siteField).selectedIndex
    ].innerText.trim()
    .toLowerCase();
  let partnerId = e.options[e.selectedIndex].value;
  let filteredData = data.filter(
    (el) => el.Site === site && el.WriterID === partnerId,
  );
  let characterList =
    filteredData.length > 0
      ? JSON.parse(filteredData[0].Characters)
      : [{ name: "NPC", id: "0" }];
...
function initTags(el, site, data) {
  let activeTags = data.filter(
    (item) =>
      JSON.parse(item.Sites).includes(site) ||
      JSON.parse(item.Sites.includes("all")),
  );
  let html = ``;
...
function initTagSites(el, data) {
  ...
  el.querySelector(".multiselect").insertAdjacentHTML(
    "beforeend",
    `<label>
...
function initCharacterSelect(el, data) {
  ...
  data.forEach((character) => {
    JSON.parse(character.Sites).forEach((site) => {
      if (
        site.site ===
        selectedSite.options[selectedSite.selectedIndex].innerText
          .trim()
          .toLowerCase()
      ) {
...
function initThreadSelect(el, data) {
  ...
  let selectedSite = el.closest("form").querySelector("#site").options[
    el.closest("form").querySelector("#site").selectedIndex
  ].value;
  let selectedCharacter = el.closest("form").querySelector("#character")
    ? el.closest("form").querySelector("#character").options[
        el.closest("form").querySelector("#character").selectedIndex
      ].value
    : null;
  let threads = [];
  ...
  if (selectedCharacter) {
    threads = [
      ...
      (item) =>
        parseInt(item.Character.id) === parseInt(selectedCharacter) &&
  ...
  if (selectedCharacter) {
    threads.sort((a, b) => {
      if (
        ...
        b.Status !== "complete" &&
        b.Status !== "archived" &&
        (a.Status === "complete" || a.Status === "archived")
      ) {
        return 1;
      } else if (a.Title < b.Title) {
        return -1;
    ...
    threads.forEach((thread, i) => {
      if (i === 0) {
        ...
        threads[i - 1].Status !== "complete" &&
        threads[i - 1].Status !== "archived" &&
        (thread.Status === "complete" || thread.Status === "archived")
      ) {
        html += `</optgroup>`;
        html += `<optgroup label="${thread.Status !== "complete" && thread.Status !== "archived" ? "Active" : "Inactive"}">`;
        html += `<option value="${thread.ThreadID}">${capitalize(thread.Title, [" ", "-", "."])}</option>`;
    ...
    threads.forEach((thread, i) => {
      if (i === 0) {
        ...
        JSON.parse(threads[i - 1].Character).name !==
        JSON.parse(thread.Character).name
      ) {
        html += `</optgroup>`;
        html += `<optgroup label="${capitalize(JSON.parse(thread.Character).name)}">`;
        html += `<option value="${thread.ThreadID}">${capitalize(thread.Title, [" ", "-", "."])}</option>`;
...
function initTagSelect(el, data) {
  ...
  data.forEach((tag) => {
    el.insertAdjacentHTML(
      "beforeend",
      `<option value="${tag.Tag}" data-sites="${JSON.parse(tag.Sites).join(", ")}">${capitalize(tag.Tag, [" ", "-"])} - ${capitalize(JSON.parse(tag.Sites).join(", "), [" ", "-"])}</option>`,
    );
  });
}
...
function initEditCharacterSelect(el, data) {
  ...
  data.forEach((character) => {
    el.insertAdjacentHTML(
      "beforeend",
      `<option value="${character.Character}">${capitalize(character.Character)}</option>`,
    );
  });
}
function initChangeBasics(el, data) {
  ...
  let existing = data.filter(
    (item) =>
      item.Character ===
      character.options[character.selectedIndex].value.trim().toLowerCase(),
  )[0];

  if (existing.Basics && existing.Basics !== "") {
    let basics = JSON.parse(existing.Basics).filter(
      (item) =>
        item.site ===
        site.options[site.selectedIndex].innerText.trim().toLowerCase(),
    );
    if (basics[0]) {
      el.closest("form")
        .querySelector("#gender")
        .setAttribute("placeholder", basics[0].basics.gender);
      el.closest("form")
        .querySelector("#pronouns")
        .setAttribute("placeholder", basics[0].basics.pronouns);
      el.closest("form")
        .querySelector("#ageValue")
        .setAttribute("placeholder", basics[0].basics.age);
      el.closest("form")
        .querySelector("#face")
        .setAttribute("placeholder", basics[0].basics.face);
      el.closest("form")
        .querySelector("#image")
        .setAttribute("placeholder", basics[0].basics.image);
      el.closest("form")
        .querySelector(".imagePreview img")
        .setAttribute("src", basics[0].basics.image);
    }
  }
}
function initAutoPopulate(el) {
  ...
  let characterExists =
    form.querySelector("#character").options[
      form.querySelector("#character").selectedIndex
    ].value !== "";
  let siteExists =
    form.querySelector("#characterSite").options[
      form.querySelector("#characterSite").selectedIndex
    ].value !== "";
  if (characterExists) {
    if (
      (siteExists && el.value !== "removeLinks") ||
      (!siteExists && el.value === "removeLinks")
    ) {
      switch (el.value) {
        case "removeLinks":
          ...
          if (!siteExists) {
            form
              .querySelectorAll(".clip-multi-warning")
              .forEach(
                (item) =>
                  (item.innerHTML = `<p>Please select both a character and a site.</p>`),
              );
          }
          break;
        case "removeShip":
    ...
    form
      .querySelectorAll(".clip-character-warning")
      .forEach(
        (item) => (item.innerHTML = `<p>Please select a character.</p>`),
      );
    form
      .querySelectorAll(".clip-multi-warning")
      .forEach(
...
function initRemoveShips(el, data) {
  ...
  let ships = JSON.parse(existing.Ships).filter(
    (item) =>
      item.site ===
      site.options[site.selectedIndex].innerText.trim().toLowerCase(),
  )[0].characters;
...
function initRemoveTags(el, data) {
  ...
  let tags = JSON.parse(existing.Tags).filter(
    (item) =>
      item.site ===
      site.options[site.selectedIndex].innerText.trim().toLowerCase(),
  )[0].tags;
...
function initCharacterSites(character) {
  let existing = storedCharacters.filter(
    (item) =>
      item.Character ===
      character.options[character.selectedIndex].value.trim().toLowerCase(),
  )[0];
...
function openSubmenu(e) {
  ...
  if (e.classList.contains("is-open")) {
    ...
    document
      .querySelector(".backdrop.horizontal")
...
function sendAjaxSync(data, form = null, count = null, num = null) {
  $.ajax({
    ...
    error: function (jqXHR, textStatus, errorThrown) {
      console.log("error");
      console.log(
        `Whoops! The sheet connection didn't quite work. Please refresh the page and try again! If this persists, please open the console (ctrl + shift + J) and send Lux a screenshot of what's there.`,
      );
      if (form) {
...
function capitalize(str, separators = [` `, `'`, `-`]) {
  ...
  let first = str.split(" ")[0].replace(regex, function (x) {
...
function addRow(e) {
  if (e.closest(".multi-buttons").dataset.rowType === "links") {
    ...
    e.closest(".adjustable")
      .querySelector(".rows")
      .insertAdjacentHTML("beforeend", formatInfoRow(e));
  } else if (e.closest(".multi-buttons").dataset.rowType === "add-appinfo") {
    e.closest(".adjustable")
      .querySelector(".rows")
      .insertAdjacentHTML("beforeend", formatAppInfoRow(e));
  } else if (e.closest(".multi-buttons").dataset.rowType === "records") {
    e.closest(".adjustable")
      .querySelector(".rows")
      .insertAdjacentHTML("beforeend", formatRecordsRow(e));
  }
}
...
function formatFeatureRow(e) {
  let site = e.closest("form").querySelector("#site").options[
    e.closest("form").querySelector("#site").selectedIndex
  ].value;
...
function handleTitleChange(currentTitle, site, data) {
  let existing = data.filter(
    (item) => item.Title === currentTitle && item.Site === site,
  )[0];
  if (existing) {
    initThreadTags(
      document.querySelector(
        '[data-form="edit-thread"] .tags.addition .multiselect',
      ),
    ...
    initThreadTags(
      document.querySelector(
        '[data-form="edit-thread"] .tags.removal .multiselect',
      ),
      JSON.parse(existing.Tags),
    );
  } else {
    document.querySelector(
      '[data-form="edit-thread"] .tags.addition .multiselect',
    ).innerHTML = `<p>This thread/site combination doesn't exist!</p>`;
    document.querySelector(
      '[data-form="edit-thread"] .tags.removal .multiselect',
    ).innerHTML = `<p>This thread/site combination doesn't exist!</p>`;
  }
...
function submitSite(form) {
  ...
  let directory =
    form.querySelector("#directory").options[
      form.querySelector("#directory").selectedIndex
    ].value;
  ...
  let close = form.querySelector("#close").value
    ? new Date(form.querySelector("#close").value.trim())
    : null;
  ...
  let data = {
    ...
    Close: close
      ? `${getMonthName(close.getMonth())} ${close.getDate()}, ${close.getFullYear()}`
      : "",
    Status: status,
  };
...
function submitTags(form) {
  ...
  let type =
    form.querySelector("#type").options[
      form.querySelector("#type").selectedIndex
    ].value;
  let tags = Array.from(form.querySelectorAll(".tag-options input")).map(
    (item) => item.value.toLowerCase().trim(),
  );
  let sites = Array.from(form.querySelectorAll('[name="site"]:checked')).map(
    (item) => item.value.toLowerCase().trim(),
  );
...
function submitPartner(form) {
  let site = form
    .querySelector("#site")
    .options[form.querySelector("#site").selectedIndex].innerText.trim()
    .toLowerCase();
...
function submitCharacter(form) {
  ...
  relationships.forEach((ship) => {
    let writer = ship.options[ship.selectedIndex].innerText
      .trim()
      .toLowerCase();
    let character = ship
      .closest(".row")
      ...
      .options[
        ship.closest(".row").querySelector("#character").selectedIndex
      ].innerText.trim()
      .toLowerCase();
    let type = ship
      .closest(".row")
      .querySelector("#type")
      .options[
        ship.closest(".row").querySelector("#type").selectedIndex
      ].innerText.trim()
      .toLowerCase();
    let section = ship
      .closest(".row")
      ...
      .options[
        ship.closest(".row").querySelector("#section").selectedIndex
      ].innerText.trim()
      .toLowerCase();
    let sectionID = ship.closest(".row").querySelector("#section").options[
      ship.closest(".row").querySelector("#section").selectedIndex
    ].dataset.id;
    shipList.push({
      writer: writer,
  ...
  let immortal = form.querySelector("#immortal").checked
    ? "immortal"
    : "mortal";
  if (tagList["age"]) {
    ...
    .then((characterData) => {
      ...
      if (existing.length === 0) {
        ...
        let data = {
          ...
          Sites: JSON.stringify(sites),
          Vibes:
            existing[0].Vibes && existing[0].Vibes !== ""
...
function submitThread(form) {
  ...
  let siteID = form
    .querySelector("#site")
    .options[form.querySelector("#site").selectedIndex].value.trim()
    .toLowerCase();
  let status = form
    .querySelector("#status")
    .options[form.querySelector("#status").selectedIndex].value.trim()
    .toLowerCase();
  let title = form.querySelector("#title").value.trim().toLowerCase();
  let character =
    form.querySelector("#character").options[
  ...
  let type = form
    .querySelector("#type")
    .options[form.querySelector("#type").selectedIndex].innerText.trim()
    .toLowerCase();
  ...
  let tags = Array.from(form.querySelectorAll(".threadTag:checked")).map(
    (item) => item.value,
  );
  ...
  featuredRows.forEach((row) => {
    featuring.push({
      name: row
        .querySelector("#character")
        .options[row.querySelector("#character").selectedIndex].innerText.trim()
        .toLowerCase()
        .replaceAll(`'`, `&apos;`),
      id: row
        .querySelector("#character")
        .options[row.querySelector("#character").selectedIndex].value.trim(),
      writer: row
        .querySelector("#partner")
        .options[row.querySelector("#partner").selectedIndex].innerText.trim()
        .toLowerCase(),
      writerId: row
        .querySelector("#partner")
        .options[row.querySelector("#partner").selectedIndex].value.trim(),
    });
  });
...
function submitApp(form) {
  let site = form
    .querySelector("#site")
    .options[form.querySelector("#site").selectedIndex].innerText.toLowerCase()
    .trim();
  let characterName = form
    .querySelector("#character")
    .options[
      form.querySelector("#character").selectedIndex
    ].innerText.toLowerCase()
    .trim();
  let characterId = form
    .querySelector("#character")
    .options[form.querySelector("#character").selectedIndex].value.trim();
  let cheatsheet = form
    .querySelector("#cheatsheet")
    .value.trim()
    .replaceAll("\n", "");
  let freeform = form
    ...
    .value.trim()
    .replaceAll("\n", "");
  let miscTitles = Array.from(form.querySelectorAll(".app-info .title")).map(
    (item) => item.value.toLowerCase().trim(),
  );
  let miscContents = Array.from(
    form.querySelectorAll(".app-info .content"),
  ).map((item) => item.value.trim().replaceAll("\n", ""));
  let miscFormatted = {};
  ...
  let existing = storedApps.filter(
    (item) => item.Character === characterName && item.Site === site,
  );
  if (existing.length > 0) {
...
function updateTags(form, data) {
  let title = form
    .querySelector("#title")
    .options[form.querySelector("#title").selectedIndex].value.trim()
    .toLowerCase();
  let newSites = Array.from(
    form.querySelectorAll(".sites .multiselect input:checked"),
  ).map((item) => item.value);
  let newTags = Array.from(form.querySelectorAll(".tag-options input")).map(
    (item) => item.value.toLowerCase().trim(),
  );
...
function updatePartner(form, data) {
  ...
  let partner = form
    .querySelector("#partner")
    .options[form.querySelector("#partner").selectedIndex].value.trim()
    .toLowerCase();
  let characters = form.querySelectorAll("#charName");
  let characterList = [];
  ...
  let existing = data.filter(
    (item) => item.Site === site && item.WriterID === partner,
  )[0];
...
function updateCharacter(form, data) {
  ...
  let site =
    siteField.options[siteField.selectedIndex].value !== ""
      ? siteField.options[siteField.selectedIndex].innerText
          .trim()
          .toLowerCase()
      : null;
  let character = form
  ...
  let selected = Array.from(
    form.querySelectorAll(".updates input:checked"),
  ).map((item) => item.value);
    ...
    if (existing.Basics && existing.Basics !== "") {
      ...
      for (instance in existingBasics) {
        if (existingBasics[instance].site === site) {
          ...
          existingBasics[instance].basics.gender =
            gender && gender !== ""
              ? gender
              : existingBasics[instance].basics.gender;
          existingBasics[instance].basics.pronouns =
            pronouns && pronouns !== ""
              ? pronouns
              : existingBasics[instance].basics.pronouns;
          existingBasics[instance].basics.age =
            age && age !== "" ? age : existingBasics[instance].basics.age;
          existingBasics[instance].basics.face =
            face && face !== "" ? face : existingBasics[instance].basics.face;
          existingBasics[instance].basics.image =
            image && image !== ""
              ? image
              : existingBasics[instance].basics.image;
          if (Object.keys(formattedExtras).length > 0) {
            existingBasics[instance].extras = {
              ...existingBasics[instance].extras,
              ...formattedExtras,
            };
          }
  ...
  if (selected.includes("changeShip")) {
    ...
    relationships.forEach((ship) => {
      ...
      let section =
        ship.querySelector("#section").options[
          ship.querySelector("#section").selectedIndex
        ];
      let type =
        ship.querySelector("#type").options[
          ship.querySelector("#type").selectedIndex
        ];
      shipList.push({
        writer: writer,
        character: character,
        relationship:
          type && type.value !== ""
            ? type.innerText.trim().toLowerCase()
            : existingType,
        section:
          section && section.value !== ""
            ? section.innerText.trim().toLowerCase()
            : existingSection,
        sectionID:
          section && section.value !== ""
            ? section.dataset.id
  ...
  if (selected.includes("addShip")) {
    ...
    for (instance in existingShips) {
      if (existingShips[instance].site === site) {
        existingShips[instance].characters = [
          ...existingShips[instance].characters,
          ...shipList,
        ];
      }
      ...
      if (existingTags[instance].site === site) {
        for (set in existingTags[instance].tags) {
          for (newSet in tagArray) {
            if (
              existingTags[instance].tags[set].type === tagArray[newSet].type
            ) {
              if (replacingTags.includes(tagArray[newSet].type)) {
                existingTags[instance].tags[set].tags = tagArray[newSet].tags;
              } else {
                existingTags[instance].tags[set].tags = [
                  ...existingTags[instance].tags[set].tags,
                  ...tagArray[newSet].tags,
                ];
              }
            } else {
  ...
  if (selected.includes("removeLinks")) {
    let remove = Array.from(form.querySelectorAll(".removeLink:checked")).map(
      (item) => ({
        title: item
          .closest("label")
          .querySelector("b")
    ...
    existingLinks = existingLinks.filter(
      (item) => item.title !== "remove" && item.url !== "remove",
    );
  ...
  if (selected.includes("removeShip")) {
    let remove = Array.from(form.querySelectorAll(".removeShip:checked")).map(
      (item) => ({
      ...
      if (existingShips[instance].site === site) {
        existingShips[instance].characters.forEach((ship) => {
          remove.forEach((removeShip) => {
            if (
              ship.character === removeShip.character &&
              ship.writer === removeShip.writer &&
              ship.relationship === removeShip.relationship
            ) {
        ...
        existingShips[instance].characters = existingShips[
          instance
        ].characters.filter(
          (item) =>
            item.character !== "remove" &&
            item.writer !== "remove" &&
            item.relationship !== "remove",
        );
      }
    }
    existing.Ships = JSON.stringify(existingShips);
  ...
  if (selected.includes("removeTags")) {
    let remove = Array.from(form.querySelectorAll(".removeTag:checked")).map(
      (item) => ({
        ...
        for (set in existingTags[instance].tags) {
          remove.forEach((tag) => {
            if (tag.type === existingTags[instance].tags[set].type) {
              existingTags[instance].tags[set].tags = existingTags[
                instance
              ].tags[set].tags.filter((item) => item !== tag.tag);
            }
          });
        }
      }
...
function updateThread(form, data) {
  let currentTitle = form
    .querySelector("#title")
    .options[form.querySelector("#title").selectedIndex].innerText.trim()
    .toLowerCase();
  let site = form
    .querySelector("#site")
    .options[form.querySelector("#site").selectedIndex].innerText.trim()
  ...
  if (selected.includes("title")) {
    existing.NewTitle = form
      .querySelector("#newTitle")
      .value.trim()
      .toLowerCase();
  } else {
  ...
  if (selected.includes("addThreadTags")) {
    let addedTags = Array.from(
      form.querySelectorAll(".tagAddition:checked"),
    ).map((item) => item.value);
  ...
  if (selected.includes("removeThreadTags")) {
    let removedTags = Array.from(
      form.querySelectorAll(".tagRemoval:checked"),
    ).map((item) => item.value);
    let existingTags = JSON.parse(existing.Tags);
...
function addRecord(form) {
  ...
  let id =
    form.querySelector("#thread-auto").options[
      form.querySelector("#thread-auto").selectedIndex
    ].value;
...
function openFilters(e) {
  ...
  if (e.closest(".filter--parent").classList.contains("is-active")) {
    document
      .querySelectorAll(".filter--parent")
      .forEach((filter) => filter.classList.remove("is-active"));
  } else {
    document
      .querySelectorAll(".filter--parent")
      .forEach((filter) => filter.classList.remove("is-active"));
            ...
            if (groupCombo[j].selected) {
              if (groupCombo[j].selected != group.selected[k]) {
                filterSelectors.push(
                  groupCombo[j].selected + group.selected[k],
                );
              }
    ...
    checkbox.addEventListener("change", (e) => {
      if (e.currentTarget.classList.contains("all")) {
        ...
        e.currentTarget.parentElement.parentElement.parentElement
          .querySelectorAll("input:not(.all)")
          .forEach((input) => {
            input.checked = false;
        ...
        if (
          ...
          e.currentTarget.parentElement.parentElement.parentElement
            ...
            .parentElement.parentElement.classList.remove("is-checked");
        }
      }
      let labels =
      ...
      if (checked === 0) {
        ...
        e.currentTarget.parentElement.parentElement.parentElement
          .querySelector("input.all")
          .parentElement.parentElement.classList.add("is-checked");
      }
      //set filters
      setCustomFilter();
    });
...
function toggleFilters(e) {
  e.closest(".filters--wrap")
    .querySelector(".filters--collapsible")
    .classList.toggle("is-open");
  if (
    !e
      .closest(".filters--wrap")
      .querySelector(".filters--collapsible")
...
function prepThreads(data, site, sites) {
  let threads =
    site !== "all"
      ? data.filter(
          (item) =>
            item.Site.trim().toLowerCase() === site &&
            item.Status.trim().toLowerCase() !== "archived",
        )
      : data.filter((item) => item.Status.trim().toLowerCase() !== "archived");
...
function populateThreads(array, siteObject) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (
      jQuery.inArray(character, characters) == -1 &&
      character != "" &&
      array[i].Status.toLowerCase() !== "complete"
    ) {
      characters.push(character);
    }

    let partnerObjects = JSON.parse(array[i].Featuring).map(
      (item) => item.writer,
    );
    partnerObjects.forEach((partner) => {
      if (
        jQuery.inArray(partner, partners) == -1 &&
        partner != "" &&
        array[i].Status.toLowerCase() !== "complete"
      ) {
    ...
    let featureObjects = JSON.parse(array[i].Featuring).map(
      (item) => item.name,
    );
    if (featureObjects) {
      featureObjects.forEach((featured) => {
        if (
          jQuery.inArray(featured, featuring) == -1 &&
          featured != "" &&
          array[i].Status.toLowerCase() !== "complete"
        ) {
    ...
    let thread = {
      ...
      site:
        siteObject.length === 1
          ? siteObject[0]
          : siteObject.filter((item) => item.Site === array[i].Site)[0],
    };
    html += formatThread(thread);
  }
  document
    .querySelector("#threads--rows")
  ...
  characters.forEach((character) => {
    ...
    let characterName =
      characterNameArr.length > 1
        ? `${character.split(" ")[0].toLowerCase()} ${character.split(" ")[1][0].toLowerCase()}`
        : character.toLowerCase();
    document
      .querySelector(".filter--characters")
      .insertAdjacentHTML(
        "beforeend",
        `<label><span><input type="checkbox" value=".${character.split(" ")[0].toLowerCase()}"/></span><b>${characterName}.</b></label>`,
      );
  });
  partners.forEach((partner) => {
    document
      .querySelector(".filter--partners")
      .insertAdjacentHTML(
        "beforeend",
        `<label><span><input type="checkbox" value=".partner--${partner.replaceAll(" ", "").toLowerCase().trim()}"/></span><b>${partner}</b></label>`,
      );
  });
  featuring.forEach((featured) => {
    let featuredArray = featured.toLowerCase().trim().split(" ");
    let featuredClass =
      featuredArray.length > 1
        ? `${featuredArray[0]}-${featuredArray[1][0]}`
        : featuredArray[0];
    let featuredName =
      featuredArray.length > 1
        ? `${featuredArray[0]} ${featuredArray[1][0]}.`
        : featuredArray[0];
    document
      .querySelector(".filter--featuring")
      .insertAdjacentHTML(
        "beforeend",
        `<label><span><input type="checkbox" value=".featured--${featuredClass}"/></span><b>${featuredName}</b></label>`,
      );
  });
  threadTags.forEach((tag) => {
    document
      ...
      .insertAdjacentHTML(
        "beforeend",
        `<label><span><input type="checkbox" value=".tag--${tag}"/></span><b>${tag}</b></label>`,
      );
  });
  if (siteObject.length > 1) {
    ...
    siteObject.forEach((site) => {
      document
        .querySelector(".filter--sites")
        .insertAdjacentHTML(
          "beforeend",
          `<label><span><input type="checkbox" value=".site--${site.ID}"/></span><b>${site.Site}</b></label>`,
        );
    });
  }
}
...
function sendThreadAjax(data, thread, form = null, complete = null) {
  $.ajax({
    ...
    complete: function () {
      console.log("complete");
      if (form) {
        form.originalTarget.querySelector('button[type="submit"]').innerText =
          "Submit";
      } else if (complete) {
...
function recordReplySend(e) {
  ...
  let site = e.currentTarget.querySelector("button").dataset.site,
    character = e.currentTarget
      .querySelector("button")
      .dataset.character.replaceAll(`'`, `&apos;`),
    ...
    words = e.currentTarget
      .closest(".record--inline")
      .querySelector(".word-count").value,
    postWords = countContentWords(
...
function prepTags(data, site) {
  ...
  let activeTags = data.filter(
    (item) => item.Sites.includes(site) || item.Sites.includes("all"),
  );
  let html = ``;
  ...
  document
    .querySelector(".characters--filters-inner")
    .insertAdjacentHTML("beforeend", html);
}
function prepCharacters(data, site, longform) {
  ...
  characters.forEach((character) => {
    let apps = [];
    if (site !== "all") {
      let entry = longform.filter(
        (item) => item.Character === character.Character && item.Site === site,
      )[0];
      if (entry) {
        apps.push({
          site: entry.Site,
          ...(entry.Cheatsheet &&
            entry.Cheatsheet !== "" && { cheatsheet: entry.Cheatsheet }),
          ...(entry.Freeform &&
            entry.Freeform !== "" && { freeform: entry.Freeform }),
          ...(entry.Misc &&
            entry.Misc !== "" && { misc: JSON.parse(entry.Misc) }),
        });
      }
    } else {
      let entries = longform.filter(
...
function formatSingleInstance(character, sites) {
  ...
  character.ships.forEach((ship) => {
    if (shipNames.includes(ship.character)) {
      ...
      if (
        parseInt(combinedShips[ship.character].sectionID) >
        parseInt(ship.sectionID)
      ) {
...
function openModal(e) {
  ...
  let modal = site
    ? e
        .closest(".has-modal")
        .querySelector(
          `.character--modal[data-type="${type}"][data-site="${site}"]`,
        )
    : e
        .closest(".has-modal")
        .querySelector(`.character--modal[data-type="${type}"]`);
...
function formatMultipleInstance(character, sites) {
  ...
  console.log(
    "sites:",
    character.sites.map((s) => s.site),
  );
  console.log(
    "basics sites:",
    character.basics.map((b) => b.site),
  );
  ...
  character.sites.forEach((siteInstance, i) => {
    let charSite = character.sites.filter(
      (item) => item.site === siteInstance.site,
    )[0];
    let basics = character.basics.filter(
      (item) => item.site === siteInstance.site,
    )[0].basics;
    let extras = character.basics.filter(
      (item) => item.site === siteInstance.site,
    )[0].extras;
    let ships = character.ships.filter(
      (item) => item.site === siteInstance.site,
    )[0].characters;
    let siteApp = character.apps.filter(
      (app) => app.site === siteInstance.site,
    )[0];
    let site = sites.filter((item) => item.Site === siteInstance.site)[0];
    ...
    .map((value, index) => {
      if (str.split(identifier).length !== index && value !== "") {
        if (
          ...
          return handleSpecialMarkdownAvoidance(
            value,
            identifier,
            opening,
            closing,
...
function createCharacterStats(data, site, sites) {
  ...
  if (site.length === 1) {
    ...
    characters = data
      .map((item) =>
        JSON.parse(item.Basics).filter(
          (instance) => instance.site === siteName,
        )[0]
          ? JSON.parse(item.Basics).filter(
              (instance) => instance.site === siteName,
            )[0].basics
...
function createThreadStats(data, site, siteID, sites) {
  ...
  let activeThreads = threads.filter(
    (item) => item.Status !== "complete" && item.Status !== "archived",
  );
  ...
  if (siteID === "all") {
    let activeSites = sites
      .filter((item) => item.Close === "")
      .map((item) => item.Site);
    activeThreads = activeThreads.filter((item) =>
      activeSites.includes(item.Site),
    );
    completedThreads = completedThreads.filter((item) =>
      activeSites.includes(item.Site),
    );
  ...
  let threadPartners = activeThreads.map((thread) =>
    JSON.parse(thread.Featuring),
  );
  let partnerNames = [];
...
function toggleRecordsView(e) {
  ...
  if (view === "heatmap") {
    e.dataset.view = "list";
    e.closest(".records").querySelector(".records--content").dataset.view =
      "list";
    let currentMonth = getMonthName(new Date().getMonth());
    document
      .querySelectorAll(`.filter--month button`)
      .forEach((button) => button.classList.remove("is-active"));
    document
      .querySelector(`.filter--month button[data-month="${currentMonth}"]`)
    ...
    document
      ...
      .forEach((button) => button.classList.remove("is-active"));
    document
      .querySelector(`.filter--month button[data-month="all"]`)
      .classList.add("is-active");
    initRecords(siteObject, staticRecords);
  }
}
...
function changeRecordFilter(e) {
  e.closest(`.filter--${e.dataset.filter}`)
    .querySelectorAll("button")
    .forEach((item) => item.classList.remove("is-active"));
  e.classList.add("is-active");
  initRecords(siteObject, staticRecords);
...
function clearSingleFilter(e) {
  document
    .querySelectorAll(
      `button[data-filter="${e.dataset.type}"][data-list-filter]`,
    )
    .forEach((button) => button.classList.remove("is-active"));
  document
    .querySelector(`button[data-filter="${e.dataset.type}"][data-all]`)
    .classList.add("is-active");
  document
    .querySelectorAll(".filter--parent")
    .forEach((item) => item.classList.remove("is-active"));
...
function clearAllFilters() {
  document
    .querySelectorAll(".threads--filter-group:has([data-list-filter]")
    .forEach((container) => {
      container
        .querySelectorAll("button[data-list-filter]")
        .forEach((button) => button.classList.remove("is-active"));
...
function filterFilters(years, characters, ships, sites, partners, multisite) {
  document.querySelectorAll('[data-filter="year"]').forEach((filter) => {
    if (
      filter.dataset.year !== "all" &&
      !years.includes(parseInt(filter.dataset.year))
    ) {
  ...
  document.querySelectorAll('[data-filter="characters"]').forEach((filter) => {
    if (
      filter.dataset.character !== "all" &&
      !characters.includes(filter.dataset.character)
    ) {
      filter.classList.add("hidden");
    } else {
      filter.classList.remove("hidden");
  ...
  document.querySelectorAll('[data-filter="partners"]').forEach((filter) => {
    if (
      filter.dataset.partner !== "all" &&
      !partners.includes(filter.dataset.partner)
    ) {
      filter.classList.add("hidden");
    } else {
      filter.classList.remove("hidden");
  ...
  if (multisite) {
    document.querySelectorAll('[data-filter="sites"]').forEach((filter) => {
      ...
      if (
        filter.dataset.site !== "all" &&
        !siteNames.includes(filter.dataset.site)
      ) {
        filter.classList.add("hidden");
      } else {
        filter.classList.remove("hidden");
...
function initRecords(sites, records, init = false) {
  let allYears = Array.from(
    document.querySelectorAll(".records .filter--year button:not([data-all]"),
  )
    .map((item) => parseInt(item.dataset.year))
    .sort((a, b) => {
      if (a > b) return -1;
  ...
  let selectedFilters = {
    year:
      document.querySelector(".records .filter--year .is-active").dataset
        .year === "all"
        ? document.querySelector(".records .filter--year .is-active").dataset
            .year
        : parseInt(
    ...
    month: document.querySelector(".records .filter--month .is-active").dataset
      .month,
    character: document.querySelector(".records .filter--characters .is-active")
      .dataset.character,
    ship: document.querySelector(".records .filter--ships .is-active").dataset
      .ship,
    site: document.querySelector(".records .filter--sites")
      ? document.querySelector(".records .filter--sites .is-active").dataset
          .site
      : sites[0].Site,
    type: document.querySelector(".records .filter--type .is-active").dataset
      .type,
    partner: document.querySelector(".records .filter--partners .is-active")
      .dataset.partner,
    metric: document.querySelector(".records .filter--metric .is-active")
      .dataset.metric,
  };
  ...
  let filteredRecords = records.filter(
    (item) =>
      ...
      (getMonthName(new Date(item.Date).getMonth()) === selectedFilters.month ||
        selectedFilters.month === "all") &&
      (JSON.parse(item.Character).name === selectedFilters.character ||
        selectedFilters.character === "all") &&
      (item.Ship === selectedFilters.ship || selectedFilters.ship === "all") &&
      ((item.threadData && item.threadData.Type === selectedFilters.type) ||
        selectedFilters.type === "all") &&
      (item.partnerNames.includes(selectedFilters.partner) ||
        selectedFilters.partner === "all"),
  );
  ...
  if (!init) {
    ...
    let relevantYears = [
      ...new Set(
        filteredRecords.map((item) => new Date(item.Date).getFullYear()),
      ),
    ];
    let relevantCharacters = [
      ...new Set(
        filteredRecords.map((item) => JSON.parse(item.Character).name),
      ),
    ];
    let relevantShips = [...new Set(filteredRecords.map((item) => item.Ship))];
    let combinedRelevantSites = [
      ...new Set(filteredRecords.map((item) => item.Site)),
    ];
    let relevantSites = [];
    siteObject.forEach((site) => {
    ...
    let combinedRelevantPartners = [
      ...new Set(filteredRecords.map((item) => item.partnerNames)),
    ];
  ...
  //format and print
  let heatmapHTML = formatHeatmap(
      filteredRecords,
      selectedFilters.year === "all" ? allYears[0] : selectedFilters.year,
...
function assessYearlyWords(records, year) {
  ...
  for (let month = 0; month < 12; month++) {
    for (let day = 0; day < daysInMonth(month, year); day++) {
      let wordsByDay = 0;
      let recordsByDay = records[getMonthName(month)].filter(
        (item) =>
...
function formatHeatmapCalendar(records, year, metric) {
  ...
  for (let i = 0; i < 12; i++) {
    html += formatMonthlyHeatmap(
      records[getMonthName(i)],
      getMonthName(i),
      year,
      max,
      metric,
...
function formatListRecord(record) {
  ...
  let formattedPartners = record.partnerNames.map(
    (item) =>
      `<button onClick="updateFilterFromRecord(this)" data-value="${item}" data-type="partner">${item}</button>`,
  );
...
function updateFilterFromRecord(e) {
  document
    .querySelector(`button[data-${e.dataset.type}="${e.dataset.value}"]`)
    .click();
}
